...
class Window(self,tk.Tk):
    ...
    def __init__(self):
        ...
        self.ent_result.place(relx=0.031, rely=0.031,
                              relheight=0.266, relwidth=0.953)
        self.ent_result.configure(
            background="white", font="TkFixedFont", justify='right')
    ...
    def init_events(self):
        ...
        # operation events.
        self.btn_add.bind(
            '<Button-1>', func=lambda event: self.key_clicked('+'))
        self.btn_sub.bind(
            '<Button-1>', func=lambda event: self.key_clicked('-'))
        self.btn_mul.bind(
            '<Button-1>', func=lambda event: self.key_clicked('*'))
